...
{   
    ...
    // compulsory type conversion operator double -> int
...
double Homeworktool::OptimalBVP(Eigen::Vector3d _start_position,Eigen::Vector3d _start_velocity,Eigen::Vector3d _target_position)
{
    ...
    */
    ...
    cout<< "matrix_eigenvalues:" << endl << matrix_eigenvalues << endl;

    vector<double> opt;     // store optimal_cost_s
    vector<double> sol;     // possible values of T

    Vector3d a(0,0,0);      // alpha
    Vector3d b(0,0,0);      // beta

    for(int i = 0; i < 4; i++){    // loop for four roots
        sol.push_back(real(matrix_eigenvalues(i)));
        double imagi = imag(matrix_eigenvalues(i));
        if(sol[i] <= 0 || abs(imagi) >= 1e-16){
            // store the great loss
            opt.push_back(optimal_cost);
            continue;
        }

        for(int j = 0; j < 3; j++){
            a(j) = (-12.0 / pow(sol[i],3)) * (_target_position(j) - _start_position(j) - _start_velocity(j) * sol[i]) - (6 /  pow(sol[i],2)) * _start_velocity(j);
            b(j) = (6.0 / pow(sol[i],2)) * (_target_position(j) - _start_position(j)- _start_velocity(j) * sol[i]) + (2 /  sol[i]) * _start_velocity(j);
        }

        // cacluate cost
        double cost = sol[i] + (1.0 / 3.0) * a.dot(a) * pow(sol[i], 2) + a.dot(b) * pow(sol[i], 2) + b.dot(b) * sol[i];
        opt.push_back(cost);
